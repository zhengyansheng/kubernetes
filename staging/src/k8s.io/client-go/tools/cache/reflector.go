...
/*
Run: 反复使用 reflector 的 ListAndWatch 去获取所有的对象 和 随后的增量 当 stopCh 管道关闭才会退出
*/
func (r *Reflector) Run(stopCh <-chan struct{}) {
	klog.V(3).Infof("Starting reflector %s (%s) from %s", r.typeDescription, r.resyncPeriod, r.name)
	// wait.BackoffUntil: 直到 ListAndWatch 主动退出，否则一直执行.
...
/*
ListAndWatch: 首先 列出所有的对象并在调用的时候获得资源版本，然后使用这个资源版本去watch操作
如果 ListAndWatch 没有初始化watch成功，它会返回错误
*/
...
func (r *Reflector) list(stopCh <-chan struct{}) error {
	...
	resourceVersion = listMetaInterface.GetResourceVersion() // meta.ResourceVersion
